...
We establish an ordinary convolutional neural network (borrowed from Keras) following the ordinary order of layers like convolutional layer - max pooling layers - dropout layers. The architecture of our CNN model is shown by figure 6, generated by the python package Keras that we use to write the codes.
...
We train the shallowest one the ResNet series, ResNet20 (borrowed from Keras). Is has 274,442 parameters and 273,066 trainable ones. Likewise, we use the same learning rate scheduler trick on this training process.
